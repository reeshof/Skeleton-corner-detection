...
template <class T> inline FIELD<T>::FIELD(int nx_, int ny_, T* v_) : nx(nx_), ny(ny_), v(v_)
{  }
...
template <class T> void FIELD<T>::thresholdInv(const T f) {
    T* curr = v;
    int end = nx * ny;
    for (int i = 0; i < end; ++i) {
        if (*curr >= f)
...
template <class T> void FIELD<T>::writePGM(const char* fname) const {
    ...
    for (const T* vend = data() + dimX() * dimY(), *vptr = data(); vptr < vend; vptr++) {
        float v = ((*vptr) - m) / (M - m); v = max(v, 0.0f);
        if (v > M) v = 1; else v = min(v, 1.0f);
...
template <class T> void FIELD<T>::draw() {
    ...
    for (int i = dimY() - 1; i >= 0; i--)
        for (const float *s = d + dimX() * i, *e = s + dimX(); s < e; s++) {
            ...
            if (draw_color) {
                std::cout << "draw color removed because linker failed for float2rgb" << std::endl;
            }
            /*    exit(-1);
            }
                //float2rgb(v, r, g, b);
