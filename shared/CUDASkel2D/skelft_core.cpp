...
{
   for(int i=0;i<size*size;++i) siteParam[i] = SKELFT_ALIVE;

   for(int j=ym;j<yM;++j)
      for(int i=xm;i<xM;++i)
      {
	     short fptr = f[i+j*size];
		   if (((thr_upper && fptr>=iso) || (!thr_upper && fptr<=iso))) siteParam[i+size*j] = SKELFT_FAR_AWAY;
	    }
   ...
   for(int j=js;j!=je;j+=jd)
      for(int i=xm;i<xM;++i)
	  {
	     float& cv = siteParam[i+size*j];
		   if (cv==SKELFT_FAR_AWAY)
			    if (siteParam[i-1+size*j]==SKELFT_ALIVE || siteParam[i+1+size*j]==SKELFT_ALIVE || siteParam[i+size*(j-1)]==SKELFT_ALIVE || siteParam[i+size*(j+1)]==SKELFT_ALIVE)
			    {
				     cv = SKELFT_NARROW_BAND;
   ...
   while(s.size())											//Process all narrowband points, add their arc-length-parameterization in siteParam
   {
      ...
      for(int dj=-1;dj<2;++dj)
		for(int di=-1;di<2;++di)
		{
		  ...
		  int ii=pt.i+di, jj=pt.j+dj;
		  if (ii>=xm && ii<xM && jj>=ym && jj<yM && siteParam[ii+size*jj]==SKELFT_NARROW_BAND)
...
float skelft2DMakeBoundary(unsigned char* f, int xm, int ym, int xM, int yM, float* siteParam, int size, bool scan_bot_to_top, short iso, bool thr_upper)
{
