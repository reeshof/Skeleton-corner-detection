...
FastMarchingMethod::FastMarchingMethod(FIELD<float>* f_,FLAGS* flags_)
		   :f(f_),flags(flags_),ptrs(f_->dimX(),f_->dimY()),N(MAXITER),verbose(false)
{
...
int FastMarchingMethod::execute(int& negd_, int& nextr_, float maxf_)
{
   ...
   for(i=0,cc=0;i<N;i++,cc++)					//Iterate until max # iterations done or all image is processed
   {
      if (!diffuse()) break;
      if (verbose && cc==step)
      { cout<<"Iteration "<<i<<" done"<<endl; cc=0; }
   }
...
void FastMarchingMethod::solve(int fi_1j,int fij_1,float vi_1j,float vij_1,float& sol)
{
	if (fi_1j == FLAGS::ALIVE || fi_1j == FLAGS::EXTREMUM)
	  ...
	  {
	     float d = 2 - SQR(vi_1j-vij_1);
	     if (d>=0)
	     {	
	       float r   = sqrt(d);
	       float ss  = ((vi_1j+vij_1) - r)/2; 
		   if (ss >= vi_1j && ss >= vij_1 && ss<sol) sol = ss;
	       else
	       {
	          ss += r;
			  if (ss >= vi_1j && ss >= vij_1 && ss<sol) sol = ss;
...
void FastMarchingMethod::tag_nbs(int min_i,int min_j,		//Gathers all neighbours (min_i,min_j) of (i,j)
								 ...
								 int& nn)
{
    if (min_i>=0 && min_i<flags->dimX() && min_j>=0 && min_j<flags->dimY() && !flags->alive(min_i,min_j))
    {
...
void FastMarchingMethod::add_to_narrowband(int i,int j,int,int)	//Adds point i,j to narrowband.
{
	...
	ptrs.value(i,j)  = map.insert(std::make_pair(f->value(i,j),Coord(i,j)));
}
