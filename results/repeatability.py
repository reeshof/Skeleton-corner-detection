...
outputPath = 'repeatabilityGraphs\\'

path = 'evaluation\\RepeatabilityScene\\'
...
for key in sceneNames:
    for detectorResult in sceneNames[key]:
        plt.plot(xvalues,detectorResult[1],label=detectorResult[0])
...
path = 'evaluation\\RepeatabilityAverage\\'
files = sorted_alphanumeric(listdir(path))
...
for key in mapValues:
    plt.plot(xvalues,mapValues[key],label=key)
plt.xlim(xvalues[0]-1,xvalues[len(xvalues)-1])
plt.ylim(0,1)
plt.legend()
plt.title("average repeatability")
